...
import {
    ...
    // eslint-disable-next-line no-unused-vars
    CircularProgress, Stack, Button, Avatar, Alert, LinearProgress
} from '@mui/material';
// eslint-disable-next-line no-unused-vars
import { Download, QrCode2, EmojiEvents, WorkspacePremium, AssignmentTurnedIn, ErrorOutline } from '@mui/icons-material';
...
const TYPE_CONFIG = {
    participation: { label: 'Participation', bg: 'linear-gradient(135deg,#1a56db,#0ea5e9)', badge: '🎓', accent: '#3b82f6' },
    winner: { label: 'Winner', bg: 'linear-gradient(135deg,#92400e,#f59e0b)', badge: '🏆', accent: '#fbbf24' },
    merit: { label: 'Merit', bg: 'linear-gradient(135deg,#5b21b6,#8b5cf6)', badge: '⭐', accent: '#8b5cf6' },
};
    ...
    return (
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.06 }}>
            <Card sx={{
                ...
                '&:hover': { boxShadow: `0 8px 40px ${tc.accent}15`, transform: isRevoked ? 'none' : 'translateY(-3px)', transition: 'all 0.2s' }
            }}>
                {/* Color accent bar */}
                ...
                <CardContent sx={{ p: 3, '&:last-child': { pb: 3 } }}>
                    <Box display="flex" justifyContent="space-between" alignItems="flex-start" mb={2}>
                        <Box>
                            <Box display="flex" alignItems="center" gap={1} mb={0.5}>
                                <Typography sx={{ fontSize: '1.5rem' }}>{tc.badge}</Typography>
                                <Chip label={tc.label} size="small"
                                    sx={{ bgcolor: `${tc.accent}15`, color: tc.accent, fontWeight: 800, border: `1px solid ${tc.accent}30`, fontSize: '0.7rem' }} />
                                {isRevoked && <Chip label="REVOKED" size="small" color="error" sx={{ fontWeight: 800, fontSize: '0.7rem' }} />}
                            </Box>
                            <Typography variant="h6" fontWeight={800} sx={{ lineHeight: 1.3 }}>{cert.event?.title}</Typography>
                            <Typography variant="caption" color="text.secondary">{cert.event?.club?.name}</Typography>
                        </Box>
                        <Avatar src={cert.event?.club?.logo_url}
                    ...
                    {cert.cert_type === 'winner' && cert.rank && (
                        <Box sx={{ p: 1.5, borderRadius: '10px', bgcolor: '#fbbf2415', mb: 2, display: 'flex', justifyContent: 'center' }}>
                            <Typography variant="h6" fontWeight={900} sx={{ color: '#f59e0b' }}>
                                {cert.rank === 1 ? '🥇' : cert.rank === 2 ? '🥈' : '🥉'} Rank #{cert.rank}
                    ...
                    {cert.cert_type === 'merit' && cert.score !== null && (
                        <Box sx={{ p: 1.5, borderRadius: '10px', bgcolor: '#8b5cf615', mb: 2 }}>
                            ...
                            <Typography fontWeight={900} sx={{ color: '#8b5cf6' }}>{cert.score}{cert.max_score ? `/${cert.max_score}` : ''} {cert.grade ? `(${cert.grade})` : ''}</Typography>
                        </Box>
                    )}
                    ...
                    <Box display="flex" justifyContent="space-between" alignItems="center">
                        <Box>
                            <Typography variant="caption" color="text.secondary" display="block">
                                {cert.certificate_number && <span style={{ fontFamily: 'monospace', fontWeight: 700 }}>{cert.certificate_number}</span>}
                            </Typography>
                            <Typography variant="caption" color="text.secondary">
                                📅 {cert.event?.start_time ? new Date(cert.event.start_time).toLocaleDateString('en-IN') : '—'}
                            </Typography>
                        </Box>
    ...
    return (
        <Box sx={{ pb: 8 }}>
            ...
            <Box component={motion.div} initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }}
                ...
                <Box sx={{ position: 'absolute', top: -100, right: -100, width: 400, height: 400, borderRadius: '50%', background: 'radial-gradient(circle, rgba(251,191,36,0.1) 0%, transparent 70%)' }} />
                <Box sx={{ position: 'relative', zIndex: 1 }}>
                    <Typography variant="h3" fontWeight={900} sx={{ letterSpacing: -1.5, mb: 1 }}>My Certificates</Typography>
                    <Typography sx={{ opacity: 0.7, fontWeight: 500 }}>Your verified achievements — downloadable, shareable, and QR-verifiable.</Typography>
                </Box>
            </Box>
            ...
            <Grid container spacing={2.5} sx={{ mb: 5 }}>
                {[
                    ...
                    <Grid item xs={6} md={3} key={s.label}>
                        <Paper sx={{ p: 2.5, borderRadius: '16px', textAlign: 'center', border: `1px solid ${s.color}20` }}>
            ...
            {certs.length === 0 ? (
                <Paper sx={{ p: 6, borderRadius: '20px', textAlign: 'center', border: '1px solid', borderColor: 'divider' }}>
                    ...
                    <Typography variant="body2" color="text.disabled">Certificates will appear here once coordinators generate them after your event attendance.</Typography>
                </Paper>
            ) : (
                <Grid container spacing={3}>
                    {/* Pie chart if multiple types */}
                    {pieData.length > 1 && (
                        <Grid item xs={12} md={4}>
                            <Paper sx={{ p: 3, borderRadius: '20px', border: '1px solid', borderColor: 'divider', height: '100%' }}>
                                <Typography variant="h6" fontWeight={900} gutterBottom>Breakdown</Typography>
                            ...
                            <AnimatePresence>
                                {certs.map((cert, i) => (
                                    <Grid item xs={12} sm={6} key={cert.id}>
                                        <CertCard cert={cert} index={i} onDownload={download} />
                                    </Grid>
                                ))}
                            </AnimatePresence>
                        </Grid>
