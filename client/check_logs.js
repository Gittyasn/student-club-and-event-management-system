...
async function check() {
    ...
    const { data, error } = await supabase.from('attendance_logs').select('*', { count: 'exact', head: true });
    if (error) {
        console.error("Table attendance_logs error:", error.message);
    } else {
        console.log("Table attendance_logs exists. Count:", data);
    }
}
